...
class Graph:
    ...
    def build_graph(self, data: List[List[int]]) -> None:
        self.graph = csr_matrix((self.V, self.V), dtype=np.int8)
    ...
    def dijkstra(
        ...
        for _ in range(n):
            ...
            ignore[u] = True
            for row in range(len(iis) - 1):
                for i in range(iis[row], iis[row + 1]):
                    if (
                        row == u
                        and not ignore[jjs[i]]
                        and dist[jjs[i]] > dist[u] + graph[i]
                    ):
